    ...
    public partial class RoomSettingsEditor : EditorContentPanel
    {
        ...
        private void RefreshLayersTree()
        {
            ...
            for (int layerIndex = 0; layerIndex < layers.Length; layerIndex++)
            {
                ...
                IAddressNode[] children = new IAddressNode[names.Count + (layer.SupportsAddAndDelete ? 1 : 0)];
                for (int index = 0; index < names.Count; index++)
                {
                    string name = names[index];
                    children[index] = new RoomEditNode(GetLayerItemUniqueID(layer, name), name, new IAddressNode[0], true, false)
                    {
                        AllowDelete = layer.SupportsAddAndDelete && index != 0 && (index == names.Count - 1)
                    };
                }
                if (layer.SupportsAddAndDelete)
                {
                    children[children.Length - 1] = new RoomEditNode(layer.DisplayName + ADD_NEW_ITEM, ADD_NEW_ITEM, new IAddressNode[0], true, true);
                }
        ...
        {
            if (currentNode == null || currentNode.DisplayName == ADD_NEW_ITEM) return;
        ...
        private void editAddressBar_SelectionChange(object sender, NodeChangedArgs e)
        {
            ...
            if (node.DisplayName == ADD_NEW_ITEM)
            {
                string newItem = layerNode.Layer.AddNewItem();
                RefreshLayersTree();
                _editAddressBar.CurrentNode = _layersRoot.GetChild(newItem, true);
                layerNode.Layer.SelectItem(newItem);
            }
            else
            {
